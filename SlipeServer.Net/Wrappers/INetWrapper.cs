...
{
    event Action<INetWrapper, uint, ushort, PacketId, byte[], uint?>? PacketReceived;

    Tuple<string, string, string> GetClientSerialExtraAndVersion(uint binaryAddress);
    IPAddress GetPlayerIp(uint binaryAddress);
    void SendPacket(uint binaryAddress, ushort port, ushort bitStreamVersion, Packet packet);
    void SendPacket(uint binaryAddress, ushort port, PacketId packetId, ushort bitStreamVersion, byte[] data, PacketPriority priority = PacketPriority.High, PacketReliability reliability = PacketReliability.ReliableSequenced);
